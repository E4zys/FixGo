...
fun DaftarScreen(
    ...
    viewModel: DaftarViewModel = koinViewModel()
) {
    ...
    LaunchedEffect(state.status) {
        when (val status = state.status) {
            ...
            is BaseState.Success -> {
                snackbarHostState.showSnackbar("Pendaftaran berhasil! Mengalihkan...")
                viewModel.clearSnackbarState()
            }
            else -> {}
        }
    ...
    BaseAuthLayout(
        title = "Daftar ke FixGo",
        subtitle = "Mulai rawat kendaraanmu dengan mudah dan nikmati perjalanan yang lebih aman.",
        modifier = modifier,
        ...
        formContent = {
            ...
            FillPasswordField(
                ...
                validators = listOf(
                    Validators.notBlank,
                    Validators.matchWith(state.password, "Konfirmasi password")
                )
            )
            ...
            Button(
                ...
                Text(if (state.status is BaseState.Loading) "Mendaftar..." else "Daftar")
            }
        },
        bottomLinkContent = {
            Row(
